...
- **点击分区**：横向左/中/右（右开本左右对调）；纵向上/中/下（菜单为居中小矩形）；条漫仅中央点按出菜单
...
- **原地切章二次确认**：翻到头再点 / 再滑 / 再按音量键一次提示，再操作一次才切章（点击与音量「再次按下…」，滑动「再次滑动…」）
- **章节内切换上一章 / 下一章**：阅读器底部工具栏直接跳转相邻章节
- **动画开关**：VP 翻页动画可独立开关
- **音量键翻页**：横/纵模式翻页，条漫模式滚动 4/5 屏高；首末页可二次确认换章
...
### 其他
- **设置入口**：点击主页底部导航的「我的」（用户个人中心），约 1 秒后页面顶部会自动插入「⚙ App扩展设置」条目，点击即可进入设置（可配置深色模式、常驻隐藏状态栏、线路探测与切换等）
- **全面屏适配（双击切换状态栏）**：主界面双击屏幕空白处即可即时隐藏或显示系统状态栏，方便快捷地适配各类挖孔、水滴与刘海等全面屏机型；亦可在设置中开启默认隐藏状态栏
- 深色模式
- 自动探测最快可用域名，后台异步更新
