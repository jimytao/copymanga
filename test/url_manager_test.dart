...
void main() {
  group('UrlManager Sticky 节点保持逻辑测试', () {
    test('当前节点为最快时，保持使用不切换', () {
      ...
      expect(
        UrlManager.shouldSwitchForSlow(valid, preferredIdx),
        isFalse,
      );
    });
    ...
    test('当前节点轻微相差（如 120ms vs 250ms），差值未达 2500ms，保持当前节点不切换', () {
      final valid = [
        ...
        const MapEntry('https://www.mangacopy.com', 250), // 虽然 250 >= 120 * 2，但差值仅 130ms
      ];
    ...
    test('当前节点极其严重滞后（如 150ms vs 3000ms），满足差值>=2500ms且倍数>=3，触发切换', () {
      final valid = [
        ...
        const MapEntry('https://www.mangacopy.com', 3000), // 差值 2850ms >= 2500ms，且 3000 >= 150 * 3
      ];
      final preferredIdx = 2; // 倒数 1 名
      expect(
        UrlManager.shouldSwitchForSlow(valid, preferredIdx),
